...
@synthesize buttonDelete,buttonIcon,labelName;

-(void)setTag:(NSInteger)tag
{
    // all subview with the same tag value
    self.buttonIcon.tag = tag;
    self.buttonDelete.tag = tag;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.buttonDelete = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, cancel_button_width, cancel_button_height)];
        [self.buttonDelete setBackgroundColor:[UIColor blackColor]];
        self.buttonDelete.center = self.buttonIcon.center;
        self.buttonDelete.clipsToBounds = NO;
        
        self.buttonDelete.titleLabel.textAlignment = UITextAlignmentCenter;
        self.buttonDelete.titleLabel.font = cancel_button_font;
